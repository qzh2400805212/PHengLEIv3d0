...
{
class StructBC;
class Param_TurbSolverStruct;

class TurbSolverStr : public TurbSolver
{
private:
...
public:
    ...
    //! To obtain the one equation turbulent viscous flux of the cell face.
    //! @param[in ]: gridIn denotes the current computational regions of the grids.
    //! @param[out ]: fluxProxy denotes the flux proxy of the cell face.
    //! @param[in ]: iSurface denotes the direction index of the grid faces, where 1 indicates the I-direction, 2 for J-direction, and 3 is K-direction.
    void CompVisFluxOneEquation(Grid *gridIn, FieldProxy *fluxProxy, int iSurface);
    ...
    //! To set the turbulent variables from primitive form to conservative form in two equation model.
    //! @param[in ]: gridIn denotes the current computational regions of the grids, in which turbulent variables are stored.
    void ChangeTurbQ (Grid *gridIn);

    //! To set the turbulent variables from conservative form to primitive form in two equation model.
    //! @param[in ]: gridIn denotes the current computational regions of the grids, in which turbulent variables are stored.
    void RecoverTurbQ(Grid *gridIn);
    ...
    //! @param[in ]: iSurface denotes the direction index of the grid faces, where 1 indicates the I-direction, 2 for J-direction, and 3 is K-direction.
    void GetInvFaceQValueforTurb(Grid *gridIn, FieldProxy *qLaminarLeftProxy, FieldProxy *qLaminarRightProxy, int iSurface);
    ...
    LIB_EXPORT Param_TurbSolverStruct *GetControlParameters();
    ...
    //! To write the flow data to restart file by original "fts" form for continue to simulation.
    //! @param[in ]: actkey contains the control information.
    void DumpRestart(ActionKey *actkey);

    //! To read the flow data from restart file by original "fts" form  for continue to simulation.
    //! @param[in ]: actkey contains the control information.
...
private:
    ...
    //! Interface: to get the turbulent viscousity.
    //! @param[in ]: gridIn denotes the current computational regions of the grids.
    void ComputeViscousCoeff(Grid *gridIn);

    //! Implement: to get the turbulent viscousity, called by function ComputeViscousCoeff.
    //! @param[in ]: gridIn denotes the current computational regions of the grids.
    void Viscosity(Grid *gridIn);
    ...
    //! To get the crossing term for two equation turbulent model.
    //! @param[in ]: gridIn denotes the current computational regions of the grids.
    void Crossing(Grid *gridIn);

    //! To get the blending term for two equation turbulent model.
    //! @param[in ]: gridIn denotes the current computational regions of the grids.
    void Blending(Grid *gridIn);

    //! Interface: To get the length scale for DES simulation.
    //! @param[in ]: gridIn denotes the current computational regions of the grids.
    void ComputeLengthScaleofOneEquationModel(Grid *gridIn);

    //! Interface: To get the length scale for SST-based DES simulation.
    //! @param[in ]: gridIn denotes the current computational regions of the grids.
    void ComputeLengthScaleofTwoEquationModel(Grid *gridIn);

    //! Implement: To get the length scale for standard DES simulation.
    //! @param[in ]: gridIn denotes the current computational regions of the grids.
    void ComputeDESLength(Grid *gridIn);

    //! Implement: To get the length scale for SST DES simulation.
    //! @param[in ]: gridIn denotes the current computational regions of the grids.
    void ComputeDESLengthSST(Grid *gridIn);

    //! Implement: To get the length scale for standard DDES simulation.
    //! @param[in ]: gridIn denotes the current computational regions of the grids.
    void ComputeDDESLength(Grid *gridIn);

    //! Implement: To get the length scale for SST DDES simulation.
    //! @param[in ]: gridIn denotes the current computational regions of the grids.
    void ComputeDDESLengthSST(Grid *gridIn);

    //! Implement: To get the length scale for standard IDDES simulation.
    //! @param[in ]: gridIn denotes the current computational regions of the grids.
    void ComputeIDDESLength(Grid *gridIn);

    //! Implement: To get the length scale for SST IDDES simulation.
    //! @param[in ]: gridIn denotes the current computational regions of the grids.
    void ComputeIDDESLengthSST(Grid *gridIn);

    //! Implement: To get the length scale for SST simulation.
    //! @param[in ]: gridIn denotes the current computational regions of the grids.
    void ComputeSSTLength(Grid *gridIn);

    //! To get the length scale for one equation model turbulent simulation.
    //! With DES: DES length scale.
    //! Without DES: wall diatance.
    //! @param[in ]: gridIn denotes the current computational regions of the grids.
    //! @return: the lenth scale for turbulent simulation.
    RDouble3D * GetLengthScale(Grid *gridIn);

    //! To get the length scale for SST model turbulent simulation.
    //! With DES: DES length scale.
    //! Without DES: sqrt(ke) / (beta * kw).
    //! @return: the lenth scale for SST turbulent simulation.
    RDouble3D * GetLengthScaleSST();

    RDouble3D * GetDESLength() { return this->DESLength; }
    RDouble3D * GetSSTLength() { return this->SSTLength; }
    void SetDESLength(RDouble3D *length) { this->DESLength = length; }
    void SetSSTLength(RDouble3D *length) { this->SSTLength = length; }
};
